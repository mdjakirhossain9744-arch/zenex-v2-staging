...
export async function GET() {
  try {
    ...
    const mnitUrl = `https://x.mnitnetwork.com/mapi/v1/mdashboard/console/info?t=${Date.now()}`;
    ...
    if (response.ok) {
      ...
      // MNIT এর গ্লোবাল ডাটা ঠিকমতো বের করা হচ্ছে
      ...
      if (rawOtps.length > 0) {
        ...
        if (bulkOps.length > 0) {
          try { await LiveLog.bulkWrite(bulkOps, { ordered: false }); } catch (e) {}
        }
      }
    }
