  ...
  const loadSummaryData = useCallback(async (isSilent = false) => {
    ...
    try {
      const res = await fetch("/api/agent-summary", {
        ...
        body: JSON.stringify({ email: parsedUser.email, role: "agent" })
      });
      ...
      <div className="p-4 md:p-10 w-full relative z-10 pb-20 font-sans">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h2 className="text-2xl font-black bg-gradient-to-r from-blue-400 to-teal-400 bg-clip-text text-transparent uppercase tracking-wider">Network Summary Report</h2>
            ...
            <p className="text-xs text-[#94A3B8] font-medium">Aggregated daily report of all users in your specific network.</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-8">
           <div className="bg-[#1E293B]/80 border border-[#334155] p-5 rounded-2xl shadow-lg border-t-2 border-t-[#3B82F6]">
              <span className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-1 block">Network Allocation</span>
           ...
           <div className="bg-[#1E293B]/80 border border-[#334155] p-5 rounded-2xl shadow-lg border-t-2 border-t-[#10B981]">
              <span className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-1 block">Total Commission</span>
           ...
           <div className="bg-[#1E293B]/80 border border-[#334155] p-5 rounded-2xl shadow-lg border-t-2 border-t-[#8B5CF6]">
              <span className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-1 block">Your Max Limit</span>
              <div className="flex items-end gap-3 mb-2"><span className="text-3xl font-black text-[#8B5CF6]">৳ {userRate.toFixed(2)}</span></div>
              <span className="text-[9px] font-black text-[#8B5CF6] bg-[#8B5CF6]/10 px-2 py-0.5 rounded tracking-widest uppercase border border-[#8B5CF6]/20">Per OTP Cap</span>
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
           ...
           <div className="bg-[#1E293B]/80 border border-[#334155] p-6 rounded-2xl shadow-lg h-[350px] flex flex-col">
              <h3 className="text-sm font-black text-white uppercase tracking-widest mb-6">Total Commission Overview</h3>
              <div className="flex-1">
                {reportData.length === 0 ? <div className="flex items-center justify-center h-full text-[#64748B] font-bold text-sm">No data available.</div> : (
                  <ResponsiveContainer width="100%" height="100%">
