  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setProfileData(prev => ({
        ...
        role: parsedUser.role || "user" // 🔥 Capture Role
      }));
      ...
      .then(data => {
        if (data.success && data.user) {
          ...
          setProfileData(prev => ({
            ...
            role: data.user.role || prev.role // 🔥 Ensure role is updated from DB
          }));
      ...
      <div className="pb-10 p-4 md:p-10 w-full relative selection:bg-[#8B5CF6] selection:text-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="lg:col-span-2 flex flex-col gap-8">
              ...
              {/* 💥 Developer API Access Card (Added Agent Logic) 💥 */}
              {profileData.role === 'agent' ? (
                <div id="api-access" className="bg-[#0F172A] border border-[#334155] p-10 rounded-3xl shadow-[inset_0_0_40px_rgba(0,0,0,0.3)] relative overflow-hidden text-center flex flex-col items-center justify-center scroll-mt-24">
                   <div className="w-20 h-20 bg-[#F43F5E]/10 rounded-full flex items-center justify-center mb-5 border border-[#F43F5E]/20 shadow-[0_0_20px_rgba(244,63,94,0.2)]">
                     <svg className="w-10 h-10 text-[#F43F5E]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                       <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                     </svg>
                   </div>
                   <h3 className="text-2xl font-black text-white tracking-widest mb-2 uppercase">No API keys found</h3>
                   <p className="text-[#94A3B8] font-medium max-w-sm">API access is restricted for Agent accounts. Only end-users and developers can generate or use API keys.</p>
                </div>
              ) : (
