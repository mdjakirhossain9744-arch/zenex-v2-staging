...
export default function AdminRealtimeConsole() {
  ...
  useEffect(() => {
    ...
    // 💥 Session Storage থেকে Pause/Freeze স্ট্যাটাস এবং ডাটা রিকভার করা 💥
  ...
  // 💥 isLive 'false' হলে SWR Key null হয়ে যাবে। এর মানে ডাটাবেসে ০% রিকোয়েস্ট! 💥
  ...
  // 💥 SWR যখনই নতুন ডাটা আনবে, তা displayData তে সেভ হবে (শুধুমাত্র Live অবস্থায়) 💥
  useEffect(() => {
    if (isLive && swrData) {
      ...
      <div className="p-4 md:p-10 w-full relative z-10 pb-20 font-sans">
        <div className="w-full">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <p className="text-[#94A3B8] text-sm font-medium tracking-widest uppercase">
                  {isLive ? `Monitoring Global Activity • ${limit} Rows` : "SYSTEM FROZEN (PAUSED)"}
                </p>
              </div>
            </div>
          ...
          <div className="mb-6 flex flex-wrap items-center gap-3 bg-[#1E293B]/80 border border-[#334155] p-3 rounded-xl shadow-lg backdrop-blur-md">
            <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} disabled={!isLive} className="bg-[#0F172A] text-xs md:text-sm font-bold text-[#94A3B8] px-4 py-2 rounded-lg border border-[#334155] focus:border-[#3B82F6] focus:text-[#E2E8F0] outline-none transition-all cursor-pointer disabled:opacity-50">
            ...
            <select value={limit} onChange={(e) => setLimit(Number(e.target.value))} disabled={!isLive} className="bg-[#0F172A] text-xs md:text-sm font-bold text-[#94A3B8] px-4 py-2 rounded-lg border border-[#334155] focus:border-[#3B82F6] focus:text-[#E2E8F0] outline-none transition-all cursor-pointer disabled:opacity-50">
