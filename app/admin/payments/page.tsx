...
export default function AdminPayments() {
  ...
  // Fetch Data (Triggered on any filter/page change)
  useEffect(() => {
    if (role === "admin") {
      fetchRealData();
    }
  ...
  const fetchPaymentSettings = async () => {
    try {
      ...
      if (data.success && data.data) { setIsWithdrawOpen(data.data.isWithdrawOpen); setMethodConfig(data.data.methods); }
    } catch (err) {}
  };
  ...
  const toggleGlobalWithdraw = async () => {
    ...
    showToast(newState ? "System OPENED" : "System CLOSED");
    await fetch("/api/payment-settings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "UPDATE", isWithdrawOpen: newState, methods: methodConfig }) });
  };
  ...
  const handleSmartBulkAction = async () => {
    ...
    const actionType = activeAdminTab === "PENDING" ? "PROCESS" : "PAID";
        ...
        <div className="w-full">
          <div className="mb-6 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
            <div>
              ...
              <p className="text-[#94A3B8] mt-1 text-sm font-medium">Enterprise Pagination Active (50/Page)</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 bg-[#0F172A] p-2 rounded-2xl border border-[#334155]">
              <div className="flex items-center gap-3 px-4 py-1 border-r border-[#334155]">
                 <span className="text-[10px] text-[#94A3B8] uppercase font-black">System Gate</span>
              ...
              {activeAdminTab === "HISTORY" && (
                <div className="px-2">
                  <select value={timeFilter} onChange={(e) => setTimeFilter(e.target.value)} className="bg-[#1E293B] border border-[#334155] text-[#10B981] font-black text-xs px-3 py-1.5 rounded-lg focus:outline-none cursor-pointer">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 bg-[#1E293B]/80 p-4 rounded-2xl border border-[#334155]">
             <div className="flex items-center gap-2 bg-[#0F172A] p-1.5 rounded-xl border border-[#334155]">
               <button onClick={() => setActiveAdminTab("PENDING")} className={`px-4 py-2 rounded-lg text-xs md:text-sm font-black transition-all ${activeAdminTab === "PENDING" ? "bg-[#3B82F6] text-white shadow-lg" : "text-[#64748B] hover:text-white"}`}>New Pending</button>
               <button onClick={() => setActiveAdminTab("PROCESSING")} className={`px-4 py-2 rounded-lg text-xs md:text-sm font-black transition-all ${activeAdminTab === "PROCESSING" ? "bg-[#EAB308] text-white shadow-lg" : "text-[#64748B] hover:text-white"}`}>Processing</button>
               <button onClick={() => setActiveAdminTab("HISTORY")} className={`px-4 py-2 rounded-lg text-xs md:text-sm font-black transition-all ${activeAdminTab === "HISTORY" ? "bg-[#10B981] text-white shadow-lg" : "text-[#64748B] hover:text-white"}`}>History</button>
             </div>
             ...
             <div className="flex w-full md:w-auto items-center gap-3">
               ...
               {(activeAdminTab === "PENDING" || activeAdminTab === "PROCESSING") && selectedIds.length > 0 && (
                 <button onClick={handleSmartBulkAction} className={`text-white font-black px-4 py-2.5 rounded-xl text-sm transition-shadow ${activeAdminTab === "PENDING" ? 'bg-gradient-to-r from-[#EAB308] to-[#CA8A04] shadow-[0_0_15px_rgba(234,179,8,0.4)]' : 'bg-gradient-to-r from-[#10B981] to-[#059669] shadow-[0_0_15px_rgba(16,185,129,0.4)]'}`}>
                   {activeAdminTab === "PENDING" ? `Process (${selectedIds.length})` : `Pay (${selectedIds.length})`}
                 </button>
               )}
          ...
          <div className="bg-[#1E293B]/80 border border-[#334155] rounded-2xl shadow-lg overflow-x-auto min-h-[300px]">
            <table className="w-full text-left whitespace-nowrap">
              <thead className="bg-[#0F172A]/50 text-[#94A3B8] uppercase text-[10px] tracking-widest border-b border-[#334155]">
                <tr>
                  {(activeAdminTab === "PENDING" || activeAdminTab === "PROCESSING") && (
              ...
              <tbody className="divide-y divide-[#334155]/50">
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="p-8 text-center text-[#64748B] font-bold">No requests found.</td></tr>
                ) : (
                  dbRequests.map((req) => (
                    <tr key={req._id} className="hover:bg-[#334155]/20 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                           <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded bg-[#334155] text-white">{req.method}</span>
                      ...
                      <td className="p-4 pr-6 text-right space-x-2">
                        {req.status === "PENDING" ? (
                          <>
                            <button onClick={() => handleAdminStatusUpdate(req._id, "PROCESSING")} className="bg-[#EAB308]/10 text-[#EAB308] hover:bg-[#EAB308] hover:text-white px-3 py-1.5 rounded-lg text-xs font-black transition-colors">Process</button>
                            <button onClick={() => handleAdminStatusUpdate(req._id, "REJECTED")} className="text-[#F43F5E] hover:underline text-xs font-black px-2">Reject</button>
                          </>
                        ) : req.status === "PROCESSING" ? (
                          <button onClick={() => handleAdminStatusUpdate(req._id, "PAID")} className="bg-[#10B981]/10 text-[#10B981] hover:bg-[#10B981] hover:text-white px-4 py-1.5 rounded-lg text-xs font-black transition-colors">Mark Paid</button>
                        ) : (
                          <div className="flex items-center justify-end gap-3">
                            ...
                            <button onClick={() => handleAdminStatusUpdate(req._id, "PENDING")} className="text-[10px] text-[#3B82F6] hover:underline font-bold">Undo</button>
                          </div>
                        )}
                      </td>
                    </tr>
