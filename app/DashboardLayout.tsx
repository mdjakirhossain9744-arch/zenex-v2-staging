    ...
    <div className="min-h-screen w-full bg-[#0F172A] text-[#E2E8F0] flex font-sans selection:bg-[#3B82F6] selection:text-white relative overflow-hidden">
      ...
      <aside className={`fixed md:relative top-0 left-0 h-screen w-64 bg-[#1E293B]/95 md:bg-[#1E293B]/90 backdrop-blur-2xl border-r border-[#334155] z-[60] shadow-2xl transition-transform duration-300 ease-in-out flex flex-col ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        <div className="h-20 flex items-center justify-between px-5 border-b border-[#334155] shrink-0">
          <Link href={dashboardUrl} prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-2 group">
        ...
        <nav className="flex-1 flex flex-col overflow-y-auto px-4 py-5 custom-scrollbar relative">
          <div className="space-y-1"> 
            ...
            {role === "admin" && (
              <>
                ...
                <Link href="/admin" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/admin' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>Control Room</Link>
                <Link href="/admin/users" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/admin/users' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>Users Directory</Link>
                <Link href="/admin/realtime" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/admin/realtime' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>Global Realtime</Link>
              </>
            )}
            ...
            {role === "agent" && (
              <>
                ...
                <Link href="/manager/users" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/manager/users' ? activeYellow : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>My Network Users</Link>
                <Link href="/manager/realtime" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/manager/realtime' ? activeYellow : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>Realtime</Link>
                <Link href="/manager/payments" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/manager/payments' ? activeYellow : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>User Payments</Link>
              </>
            )}
            ...
            <Link href={dashboardUrl} prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${isDashboardActive ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>Dashboard</Link>
            
            {role === "user" && (
              <Link href="/get-number" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/get-number' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>Get Number</Link>
            )}

            <Link href="/console" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/console' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg>Console</Link>
            <Link href="/top-user" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/top-user' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>Top User</Link>
            <Link href={summaryUrl} prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${isSummaryActive ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>Summary</Link>

            <p className="px-3.5 text-[10px] font-bold tracking-widest text-[#94A3B8] mt-5 mb-2 uppercase">Account & Tools</p>
            
            {role === "admin" ? (
               <Link href="/admin/payments" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/admin/payments' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>Payments</Link>
            ) : (
               <Link href="/withdraw" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/withdraw' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>Withdraw</Link>
            )}

            <Link href="/master-range" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/master-range' ? activeBlue : inactive}`}>
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
              Master Range
            </Link>

            <Link href="/access-list" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/access-list' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>Access List</Link>
            <Link href="/profile" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/profile' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>Profile</Link>
            <Link href="/notifications" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${pathname === '/notifications' ? activeBlue : inactive}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>Notifications</Link>
          </div>
          ...
          <div className="mt-auto pt-6 pb-2 px-1 shrink-0">
             <div className="bg-gradient-to-br from-[#0F172A] to-[#1E293B] border border-[#3B82F6]/30 rounded-xl p-3 shadow-md relative overflow-hidden group">
                ...
                <svg className="w-4 h-4 text-[#3B82F6] mb-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <Link href="/api-docs" prefetch={false} onClick={() => setIsMobileMenuOpen(false)} className="block w-full py-1.5 bg-[#3B82F6]/10 hover:bg-[#3B82F6]/20 border border-[#3B82F6]/30 rounded-md text-center text-[9px] font-black tracking-widest text-[#3B82F6] uppercase transition-colors">
                   View API Docs
                </Link>
             </div>
          </div>
      ...
      <main className={`flex-1 flex flex-col h-screen overflow-hidden w-full relative ${isMaintenance && role === 'admin' ? 'mt-6' : ''}`}>
        <header className="h-16 md:h-20 bg-[#1E293B]/80 backdrop-blur-2xl border-b border-[#334155] flex items-center justify-between px-3 sm:px-4 md:px-10 z-[50] w-full shrink-0 relative">
          <div className="flex items-center gap-2 sm:gap-3">
             ...
             <Link href={dashboardUrl} prefetch={false} className="md:hidden flex items-center gap-1.5 group shrink-0">
            ...
            <div className="relative shrink-0">
              ...
              {isProfileMenuOpen && (
                <div className="absolute top-12 right-0 w-56 bg-[#1E293B] border border-[#334155] rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] z-50 overflow-hidden flex flex-col">
                  ...
                  <Link href="/profile" prefetch={false} onClick={() => setIsProfileMenuOpen(false)} className="w-full flex items-center gap-3 px-5 py-3.5 text-sm font-bold text-[#E2E8F0] hover:bg-[#334155]/30 transition-colors">
