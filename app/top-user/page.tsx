...
export default function TopUser() {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.settings) {
          ...
          setBannerPrize(data.settings.bannerPrize);
        }
      }
    } catch (err) {
  ...
  const saveBannerSettings = async () => {
    ...
    try {
      const res = await fetch("/api/leaderboard", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "UPDATE_BANNER", bannerActive, bannerText, bannerPrize })
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <DashboardLayout>
        ...
        {role === "admin" && (
          <div className="mb-10 bg-[#1E293B]/80 border border-[#3B82F6]/50 p-6 rounded-2xl shadow-lg border-t-4 border-t-[#3B82F6]">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <div>
                   <label className="block text-xs font-bold text-[#94A3B8] mb-2 uppercase">Banner Text</label>
                ...
                <div>
                   <label className="block text-xs font-bold text-[#94A3B8] mb-2 uppercase">Prize Amount/Reward</label>
                ...
                <div className="flex items-center gap-4">
                   <label className="flex items-center gap-3 cursor-pointer">
                      <div className="relative">
                        ...
                        <div className={`block w-14 h-8 rounded-full transition-colors ${bannerActive ? 'bg-[#10B981]' : 'bg-[#334155]'}`}></div>
                        <div className={`dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform ${bannerActive ? 'transform translate-x-6' : ''}`}></div>
                      </div>
                      <span className="text-sm font-bold text-white uppercase">{bannerActive ? "Offer is Live" : "Offer is OFF"}</span>
                   </label>
                   <button onClick={saveBannerSettings} disabled={isSaving} className="ml-auto bg-[#3B82F6] hover:bg-[#2563EB] text-white px-6 py-2.5 rounded-lg font-black text-sm transition-all disabled:opacity-50">
      ...
      {!loading && role === "user" && myRank.rank > 0 && (
